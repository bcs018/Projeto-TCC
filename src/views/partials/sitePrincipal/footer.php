<footer class="footer">
    <div class="container-fluid px-lg-5">
        <div class="row">
            <div class="col-md-9 py-5">
                <div class="row">
                    <div class="col-md-4 mb-md-0 mb-4">
                        <h2 class="footer-heading">Sobre nós</h2>
                        <p>Somos uma pequena startup pensando em solucionar a dificuldade em criação de lojas virtuais
                            para pequenos empreendedores e micro empresas.</p>
                        <ul class="ftco-footer-social p-0">
                            <li class="ftco-animate"><a href="#" data-toggle="tooltip" data-placement="top"
                                    title="Twitter"><span class="fa fa-twitter"></span></a></li>
                            <li class="ftco-animate"><a href="#" data-toggle="tooltip" data-placement="top"
                                    title="Facebook"><span class="fa fa-facebook"></span></a></li>
                            <li class="ftco-animate"><a href="#" data-toggle="tooltip" data-placement="top"
                                    title="Instagram"><span class="fa fa-instagram"></span></a></li>
                        </ul>
                    </div>
                    <div class="col-md-8">
                        <div class="row justify-content-center">
                            <div class="col-md-12 col-lg-10">
                                <div class="row">
                                    <div class="col-md-4 mb-md-0 mb-4">
                                        <h2 class="footer-heading">Como fazer sua loja</h2>
                                        <ul class="list-unstyled">
                                            <li><a href="#" class="py-1 d-block">Cadastro</a></li>
                                            <li><a href="#" class="py-1 d-block">Criação da loja</a></li>
                                            <li><a href="#" class="py-1 d-block">Mudando o template</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row mt-md-5">
                    <div class="col-md-12">
                        <p class="copyright">
                            <!--Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
                            Copyright &copy;<script>
                                document.write(new Date().getFullYear());
                            </script> Todos os direitos reservados | BW Commerce <br>
                            modelo feito por <a href="https://colorlib.com" target="_blank">Colorlib.com</a>
                            <!--Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-md-3 py-md-5 py-4 aside-stretch-right pl-lg-5">
                <img alt="Logo BW commerce" src="<?php echo BASE_ASS; ?>images/logoB.png" id="logo" width="210px" height="55px">
            </div>
        </div>
    </div>
</footer>

<!-- loader -->
<div id="ftco-loader" class="show fullscreen">
    <svg class="circular" width="48px" height="48px">
        <circle class="path-bg" cx="24" cy="24" r="22" fill="none" stroke-width="4" stroke="#eeeeee" />
        <circle class="path" cx="24" cy="24" r="22" fill="none" stroke-width="4" stroke-miterlimit="10" stroke="#F96D00" />
    </svg>
</div>

<script src="<?php echo BASE_ASS; ?>js/jquery.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/jquery-migrate-3.0.1.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/popper.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/bootstrap.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/jquery.easing.1.3.js"></script>
<script src="<?php echo BASE_ASS; ?>js/jquery.waypoints.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/jquery.stellar.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/jquery.animateNumber.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/owl.carousel.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/jquery.magnific-popup.min.js"></script>
<script src="<?php echo BASE_ASS; ?>js/scrollax.min.js"></script>
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBVWaKrjvy3MaE7SQ74_uJiULgl1JY0H2s&sensor=false">
</script>
<script src="<?php echo BASE_ASS; ?>js/google-map.js"></script>
<script src="<?php echo BASE_ASS; ?>js/main.js"></script>
<script src="<?php echo BASE_ASS; ?>js/sitePrincipal/validaCadastro.js"></script>
<script src=" https://kit.fontawesome.com/2ebeb81d35.js"></script>

</body>

</html>